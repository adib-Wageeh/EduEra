...
void main() {
  ...
  test('should return right hand side List<Exam>', () async{

    when(()=> examRepository.getExams(any()),
    ).thenAnswer((_) async=> const Right([]));

    final result = await getExamsUseCase(tCourseId);

    expect(result, const Right<dynamic,List<Exam>>([]));
    verify(() => examRepository.getExams(any())).called(1);
