...
import 'package:education_app/src/notification/presentation/views/notifications_view.dart';
  ...
  void initState() {
    ...
    newNotificationListenable.addListener(() {
      if(newNotificationListenable.value){
        if(!context.read<NotificationsNotifier>().getMuteState)
        {
          player.play(
        ...
        if(state is NotificationsLoaded){
          final unseenNotificationsLength =
            state.notifications.where
              ((notification) => notification.seen == false).length;
